...
export async function ProductMetrics({ searchParams }: { searchParams: SearchParams }) {
  ...
  const [totalSkus, lowStockCount, totalUnitsAgg, inventoryValueData] = await Promise.all([
    prisma.product.count({ where }),
    prisma.product.count({ where: { ...where, quantity: { lte: 5 } } }),
    prisma.product.aggregate({ _sum: { quantity: true }, where }),
    prisma.product.findMany({ where, select: { quantity: true, cost: true } })
  ]);

  const totalUnits = totalUnitsAgg._sum.quantity ?? 0;
  const inventoryValue = inventoryValueData.reduce(
    (acc, p) => acc + p.quantity * Number(p.cost ?? 0),
    0
  );
